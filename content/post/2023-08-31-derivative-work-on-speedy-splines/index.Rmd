...
functions {
  ...
  vector build_b_spline(vector x, array[] real ext_knots, 
    ...
    vector[n_x] b_spline;
    vector[n_x] w1 = rep_vector(0, n_x);
    vector[n_x] w2 = rep_vector(0, n_x);

    // first derivative & weights
    vector[n_x] d1;
    vector[n_x] f1 = rep_vector(0, n_x);
    vector[n_x] f2 = rep_vector(0, n_x);
    vector[n_x] f3 = rep_vector(0, n_x);
    vector[n_x] f4 = rep_vector(0, n_x);
    
    // second derivative & weights
    vector[n_x] d2;
    vector[n_x] s1 = rep_vector(0, n_x);
    vector[n_x] s2 = rep_vector(0, n_x);
    vector[n_x] s3 = rep_vector(0, n_x);
    vector[n_x] s4 = rep_vector(0, n_x);
    
    if (k == 1) {
      
      // first order splines
      for (i in 1:n_x) {
        b_spline[i] = (ext_knots[ind] <= x[i]) 
                    && (x[i] < ext_knots[ind + 1]); 
        d1[i] = 0;
        d2[i] = 0;
      }
      ...
      b_spline = w1 .* build_b_spline(x, ext_knots, ind, k - 1, 0) 
...
transformed data {
  ...
  int k = degree + 1; // k is order of spline
}
